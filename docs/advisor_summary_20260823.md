...
| **B+ v5** | + VLM 逐帧 hidden state | 让 VLM 真正感知每一帧（喂全部 20 帧，池化逐帧 LLM 特征 [T,2560]）——VLM 对每帧的直接理解，比 SAM2 均值池化强一个量级 |
...
| **B+ v5**（VLM 全帧感知,pw1） | **4.8%** | 55.7% | 63.2% | 1.0% | 2.8% |
| B+ v5（pw0.7 变体） | 4.8% | 54.5% | 64.0% | 1.0% | 2.7% |
| **v6 头**（first-present anchor + 差分特征,thr=0.7） | 4.5% | **50.6%** | **54.7%** | 1.0% | 2.8% |
| **v6 头**（thr=0.8，幻觉优先操作点） | **4.0%** | **43.5%** | **46.1%** | 1.0% | 2.7% |
...
- **B+（VLM 全帧感知）**：VLM 亲眼看每帧 → 判断"目标还在"最准——overall present_miss **5.8%**、frame_acc **95.0%**（全版本最高），temporal 漏检 11.9%（vs v2 的 26.7%）；且在 temporal 幻觉（55.7%<67.5%）和漏检（11.9%<15.6%）上同时优于 TEG 基线
- **权衡仍在**（幻觉 vs 漏检），但 B+ 把整个权衡曲线往下推了一大截——操作点覆盖"幻觉优先（v2: 37.6%）"到"可用优先（B+: miss 5.8%）"
- **pos_weight=0.7 变体与 pw=1.0 几乎相同**（temporal 54.5% vs 55.7%）——VLM 特征主导，pw 影响小；**最终采用 pw=1.0**（漏检更低：11.9% vs 13.9%）
- **最终模型建议**：B+ v5（pw=1.0）作为"可用"最终模型（漏检 5.8%、frame_acc 95.0% 全版本最优，幻觉同时优于所有无忠实性基线）；v2（temporal 37.6%）作为"最佳幻觉"报告点，论文里说明权衡曲线
- **v6 头（2026-08-25 新增）**：修复两个关键缺陷后重训——① anchor 从"第 0 帧"改为"目标首次出现帧"（eval 里 49% 的 temporal case 第 0 帧目标不在，旧 anchor 失效）；② 加入显式锚点差分特征（mask 区域 / VLM 感知 vs 锚点）。结果：**identity 幻觉 63.2%→46.1%（-17pp）**、**temporal 55.7%→43.5%（-12pp）**（thr=0.8），代价是漏检上升（temporal 11.9→20.2%）——形成清晰 risk-coverage 权衡曲线（thr 0.4-0.8），论文可展示操作点选择
- **诊断结论**：VLM 单帧验证精度 70-83%（目标消失后会把"相似目标"当目标）；锚点 cos 判别力 AUC 0.71-0.74（目标移动/光照也导致外观漂移）——纯推理 TVR（段尾/锚点触发验证）只能改善 1-2pp，必须把"锚点差分"信息**融入训练**（v6）才能真正突破
...
| **B+：VLM 感知所有帧** | ✅ 完成（overall miss 5.8%、frame_acc 95.0%） |
| B+ pos_weight 变体 | ✅ 完成（pw 影响小，采用 pw=1.0） |
| 外部基线（SESAME/GSVA/MeViSv2/HalluSegBench） | ✅ 完成 |
| Figure 1 + 失败分类学 | ✅ 完成 |
| 对比视频网页（模型效果展示） | ✅ 已部署（http://172.18.127.61:8899） |

**下一步**：
1. ✅ v6 头（anchor 修复 + 差分特征）——identity -17pp、temporal -12pp（thr=0.8），已集成为模型默认时序头
2. 选定最终操作点（thr 0.6-0.8 权衡）→ 更新对比视频网页
