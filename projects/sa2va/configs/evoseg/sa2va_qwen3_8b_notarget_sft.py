"""EvoSeg no-target (abstain) SFT -- 8B Faithful (same recipe as 4B).

LoRA-fine-tunes the 8B-MultiTask on a faithfulness-balanced mix: gRefCOCO
train no-target abstain data (majority, x4) + RefCOCO / gRefCOCO present (x1).
8B was trained from scratch (no Sa2VA-8B pretrained pth); we continue from the
8B-MultiTask xtuner checkpoint (weights-only).

Continue from:
  evo_artifacts/checkpoints/8b_multitask_iter50768_weights.pth
"""
...
continue_pth = ('/9950backfile/chenjiahui/evo_artifacts/checkpoints/'
                '8b_multitask_iter50768_weights.pth')
