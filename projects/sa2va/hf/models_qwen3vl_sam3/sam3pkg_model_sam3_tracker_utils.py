...
try:
    # Triton-based EDT, only used by the training-time click sampler
    # (`sample_box_points` / `get_next_point`), not on the inference path Sa2VA uses.
    raise NotImplementedError('model.edt op not vendored in the HF release; fill_hole_area must be 0')  # edt_triton
except Exception:  # noqa: BLE001 - triton may be absent in the Sa2VA env
    edt_triton = None
...
def _get_connected_components_with_padding(mask):
    ...
    raise NotImplementedError('perflib.connected_components op not vendored in the HF release; fill_hole_area must be 0')  # connected_components
